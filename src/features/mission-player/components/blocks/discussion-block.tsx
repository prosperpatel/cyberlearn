...
export function DiscussionBlock({ block }: { block: StandardBlock }) {
  ...
  return (
    <div className="rounded-xl border border-teal-500/30 bg-base-900 overflow-hidden">
      <div className="h-1.5 w-full bg-gradient-to-r from-teal-500 to-teal-500/20" />

      <div className="px-5 sm:px-8 py-6 space-y-5">
        {/* Header */}
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1.5 rounded-md bg-teal-500/10 border border-teal-500/20 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-teal-400 font-mono">
        ...
        {/* Main prompt */}
        <blockquote className="border-l-2 border-teal-500/60 pl-4 text-sm text-foreground/90 leading-relaxed italic">
          {prompt}
        </blockquote>
            ...
            <ol className="space-y-2">
              {guideQuestions.map((q, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-foreground/80">
                  <span className="shrink-0 mt-0.5 flex size-5 items-center justify-center rounded-full bg-teal-500/15 text-[10px] font-bold text-teal-400 font-mono">
                    {i + 1}
                  </span>
